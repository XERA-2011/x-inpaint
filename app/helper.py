...
import numpy as np
import torch
from app.const import MPS_UNSUPPORT_MODELS
...
#     model_path = r'.cache\torch\hub\checkpoints\big-lama.pt'
#     try:
...
def load_jit_model(url_or_path, device, model_md5: str):
    ...
    try:
        ...
        os.chdir(model_dir)
        logger.info(f"Loading model from: {model_path}")
        model = torch.jit.load(os.path.basename(model_path), map_location="cpu").to(device)
    except Exception as e:
        handle_error(model_path, model_md5, e)
    finally:
        # 恢复原始工作目录
